...
export function compareTransportOptions(distanceKm: number): TransportOption[] {
  ...
  const modes: Record<string, { speedKmh: number, co2PerKm: number, costPerKm: number, baseCost: number }> = {
    metro: { speedKmh: 40, co2PerKm: 0.04, costPerKm: 0, baseCost: 2.5 },
    bus: { speedKmh: 20, co2PerKm: 0.08, costPerKm: 0, baseCost: 2.0 },
    rideshare: { speedKmh: 30, co2PerKm: 0.20, costPerKm: 1.5, baseCost: 5.0 },
  ...
  for (const [mode, metrics] of Object.entries(modes)) {
    ...
    const costEstimate = Number((metrics.baseCost + distanceKm * metrics.costPerKm).toFixed(2));
