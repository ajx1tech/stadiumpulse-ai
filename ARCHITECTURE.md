...
- **AI Integration**: **Google Gemini API** is called client-side via a `NEXT_PUBLIC_GEMINI_API_KEY`. 
  - *Note on Tradeoffs*: Calling the API directly from the client is acceptable for the scope of this hackathon to demonstrate low-latency interactions. In a production environment, this would be proxied through a Vercel Serverless Function (API Route) to completely hide the API key from the browser.
