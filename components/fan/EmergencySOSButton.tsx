...
import { validateIncidentReport } from '@/lib/validation'
 ...
 */
export default function EmergencySOSButton(): import('react').JSX.Element {
  ...
  const handleSOS = async () => {
    ...
    const validationResult = validateIncidentReport(incident)
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) setValidationError(null); }}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[101] w-full max-w-md translate-x-[-50%] translate-y-[-50%] bg-slate-900 border-2 border-red-600 rounded-xl p-6 shadow-2xl focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
          <Dialog.Title className="text-2xl font-extrabold text-white mb-2 uppercase tracking-wide flex items-center">
            <span className="text-red-500 mr-2 text-3xl" aria-hidden="true">
              ⚠️
            </span>
            Confirm Emergency
          </Dialog.Title>
          <Dialog.Description className="text-slate-300 text-lg mb-4">
            Are you sure you need emergency assistance? This will dispatch
            medical and security personnel to your immediate area.
          </Dialog.Description>

          {validationError && (
            <div className="mb-4 text-red-400 font-bold" role="alert" aria-live="polite">
              {validationError}
            </div>
          )}
          ...
          {submitted ? (
            <div
              className="bg-emerald-900/50 border border-emerald-500 rounded p-4 text-emerald-400 font-bold text-center animate-pulse"
              role="alert"
            >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-end">
              <Dialog.Close asChild>
                <button className="px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-lg focus:outline-none focus:ring-2 focus:ring-white">
                  Cancel
                </button>
              </Dialog.Close>
              <button
