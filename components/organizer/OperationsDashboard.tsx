 ...
 */
export default function OperationsDashboard(): import('react').JSX.Element {
  ...
  useEffect(() => {
    const unsubTelemetry = subscribeToLiveTelemetry((data) =>
      setTelemetry(data)
    )
  ...
  const chartData =
    telemetry.length > 0
      ? telemetry
          .slice(0, 20)
          .reverse()
          .map((t, i) => ({ time: `T-${20 - i}`, density: t.densityPercent }))
      : [
          { time: 'T-10', density: 30 },
          { time: 'T-9', density: 35 },
          { time: 'T-8', density: 40 },
          { time: 'T-7', density: 38 },
          { time: 'T-6', density: 45 },
    ...
    <div className="bg-slate-900 min-h-screen text-slate-100 p-6">
      <header className="mb-8 border-b border-slate-700 pb-4">
        <h1 className="text-3xl font-extrabold text-emerald-400 tracking-tight">
          Organizer Operations Command Center
        </h1>
            ...
            <div className="bg-slate-900 p-4 rounded-lg min-h-[100px] border border-slate-700">
              {briefing ? (
                ...
                <p className="text-slate-500 italic">
                  Click generate to request a Gemini-powered summary of active
                  incidents.
                </p>
              )}
            </div>
          ...
          >
            {incidents.length === 0 ? (
              ...
              incidents.map((inc) => (
                <div
                  key={inc.id}
                  className="p-3 bg-slate-700 rounded-lg border-l-4 border-red-500"
                >
